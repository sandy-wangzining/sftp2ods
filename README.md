...
sftp2ods --job jobs/my_sftp.json --bizdate 20260920 --force      # 强制重写（忽略台账；也用于确认要写空分区）
```
...
| `skip_if_empty` | - | 这些列（**目标列名**）值为空的行跳过不入库（如 `["order_id"]`），跳过的行数会打日志（`''` 与 NULL 都算空）。不能指向第一列（与 `footer` 冲突，配置阶段报错） |
| `footer` | - | 开启合计行处理：首列为空的合计行不写库；`{"sum": ["金额列", ...]}` 会校验「合计 = 入库行之和」（被 `skip_if_empty` 跳过的行不计入），对不上报错（sum 只允许 decimal 列） |
...
> （宁可失败，不写错数）；`decimal` 允许千分位逗号（`1,234.50`），但逗号不是千分位写法时
> 报错（欧式小数 `1.234,56` 会被明确拒绝，而不是静默缩小 1000 倍）；
> 取值必须落在列类型允许的范围内（decimal 的小数位/整数位、bigint 的 64 位），超了在写库前报错。
...
回退按旧文件名查找**只用于判定「已上传」**（台账里那个键的 pt 必须就是这一天）；
下载与写库一律用 `下载目录/日期/文件名`，不会把旧目录里别的日期的同名文件当成本日数据
（文件名不含日期时尤其重要）。
...
11. 补数区间与远端没有任何交集（`--start-date` 写到了数据范围之外）时直接失败，不会静默「空跑成功」；
12. 解析出 0 行时先查该分区现有行数：非 0 则失败（`--force` 才写空分区），避免源文件被截断成
    只剩表头、或整列被 `skip_if_empty` 滤光时把已有数据清空；
13. 远端返回的文件名/目录名含路径分隔符或 `..` 时直接拒绝（这类名字会被拼进本地路径，
    逃出下载目录写到任意位置）；
14. 软链文件按 `stat` 跟随链接取真实大小（READDIR 给的是链接自身长度，直接拿来核对会永远"大小不一致"）。

### 退出码（调度侧判断成败）

| 码 | 含义 |
|---|---|
| 0 | 成功（含 `--dry-run`；`--check` 是体检，缺文件只提示、不判失败） |
| 1 | 运行失败：配置错、连接/下载失败、解析失败、缺文件、日期范围与远端没有交集、写库失败、写后行数对不上 |
| 2 | 命令行参数问题（缺 `--job`、日期写法错、`--bizdate` 与区间互斥、负数超时等），**没做过任何远端操作** |
| 130 | 用户中断（Ctrl+C），同步与 `--init` 一致 |
